const users = {
  marvin: { name: "malazagamarvin", image: "image/users/malazagamarvin.jpg" },
...
function validateScan(scannedCode) {
  ...
  if (html5Scanner) {
    html5Scanner.clear();
    html5Scanner = null;
    scannerContainer.innerHTML = ''; // Clean up DOM
  }
}
...
document.getElementById('scan-btn').addEventListener('click', () => {
  if (!lockedUser) {
    message.textContent = "Please lock a user first.";
    return;
  }

  scannerContainer.style.display = 'block';
  scannerContainer.innerHTML = ''; // reset container

  html5Scanner = new Html5QrcodeScanner("scanner", { fps: 10, qrbox: 250 });
  html5Scanner.render((decodedText) => {
    validateScan(decodedText);
  });
});
